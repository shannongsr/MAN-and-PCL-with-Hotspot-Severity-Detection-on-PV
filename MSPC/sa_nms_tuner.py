#!/usr/bin/env python
# -*- coding: utf-8 -*-

"""
Simulated Annealing (SA) tuner for ORIGINAL hard NMS on Ultralytics val().

Optimizes (conf, nms_iou, max_det) to maximize a chosen metric (default: mAP50-95).
Fair-comparison protocol matches the WBF and Soft-NMS(Linear) SA tuners:
- Monkey-patch ultralytics.utils.ops.non_max_suppression so our chosen (conf, iou, max_det)
  are guaranteed to be used during validation, regardless of internal defaults.
- No extra re-ranking or fusion: this is the vanilla hard NMS.

Usage example:
python sa_nms_tuner.py \
  --weights runs/detect/xxx/weights/best.pt \
  --data test_splits/test_part_001/data.yaml \
  --imgsz 640 --batch 4 \
  --metric map5095 \
  --iters 60 --temp0 1.0 --alpha 0.90 --steps-per-temp 5 \
  --conf-range 0.001 0.3 \
  --nms-iou-range 0.3 0.8 \
  --max-det-range 100 500 \
  --seed 42 \
  --project runs/sa_nms --name coffee_part001_sa

Outputs:
- CSV log with all trials (same columns as WBF/Soft-NMS tuners)
- best_nms.yaml (same schema as other tuners)
"""

import argparse
import csv
import math
import random
from pathlib import Path

import numpy as np
import torch
import yaml
from ultralytics import YOLO


# ----------------------------- NMS patch (vanilla hard NMS) -----------------------------

_ORIG_NMS_FN = None

def install_hard_nms_patch(conf_thres: float, iou_thres: float, max_det_out: int):
    """
    Monkey-patch ultralytics.utils.ops.non_max_suppression to enforce our (conf, iou, max_det).
    This keeps the algorithm exactly as original hard NMS, only fixing the hyperparameters.
    """
    global _ORIG_NMS_FN
    from ultralytics.utils import ops
    if _ORIG_NMS_FN is None:
        _ORIG_NMS_FN = ops.non_max_suppression  # store original once

    def _patched_non_max_suppression(
        prediction,
        conf_thres_arg=0.25,
        iou_thres_arg=0.45,
        classes=None,
        agnostic=False,
        multi_label=False,
        labels=(),
        max_det=300,
        nc=0,
        max_time_img=0.05,
        max_nms=30000,
        max_wh=7680,
        in_place=True,
        rotated=False,
        **kwargs
    ):
        # We deliberately override conf_thres / iou_thres / max_det with our tuner values
        return _ORIG_NMS_FN(
            prediction,
            conf_thres=conf_thres,     # enforce tuner conf
            iou_thres=iou_thres,       # enforce tuner IoU
            classes=classes,
            agnostic=agnostic,
            multi_label=multi_label,
            labels=labels,
            max_det=max_det_out,       # enforce tuner max_det
            nc=nc,
            max_time_img=max_time_img,
            max_nms=max_nms,
            max_wh=max_wh,
            in_place=in_place,
            rotated=rotated,
            **kwargs
        )

    ops.non_max_suppression = _patched_non_max_suppression


# ----------------------------- SA utilities (same style as other tuners) -----------------------------

def clamp(v, lo, hi):
    return max(lo, min(hi, v))

def propose(current, ranges, step_scales):
    """
    Neighborhood proposal: gaussian noise for conf/iou; integer noise for max_det.
    """
    conf = clamp(
        current['conf'] + random.gauss(0, step_scales['conf']),
        ranges['conf'][0], ranges['conf'][1]
    )
    iou = clamp(
        current['nms_iou'] + random.gauss(0, step_scales['nms_iou']),
        ranges['nms_iou'][0], ranges['nms_iou'][1]
    )
    max_det = int(round(
        clamp(current['max_det'] + random.gauss(0, step_scales['max_det']),
              ranges['max_det'][0], ranges['max_det'][1])
    ))
    return {'conf': conf, 'nms_iou': iou, 'max_det': max_det}

def accept_prob(old, new, T):
    if new >= old:
        return 1.0
    return math.exp((new - old) / max(1e-9, T))

def fetch_metric(results, metric_key: str):
    score = None
    if hasattr(results, 'results_dict') and isinstance(results.results_dict, dict):
        rd = results.results_dict
        mapping = {
            'map5095': ('metrics/mAP50-95(B)', 'mAP50-95', 'map'),
            'map50':   ('metrics/mAP50(B)', 'mAP50', 'map50'),
            'map75':   ('metrics/mAP75(B)', 'mAP75', 'map75'),
            'precision': ('metrics/precision(B)', 'P', 'mp'),
            'recall':    ('metrics/recall(B)', 'R', 'mr'),
        }
        keys = mapping.get(metric_key, ('mAP50-95', 'map'))
        for k in keys:
            if k in rd and rd[k] is not None:
                score = float(rd[k])
                break
    if score is None:
        try:
            m = results.metrics
            if metric_key == 'map5095':
                score = float(getattr(m, 'map', None))
            elif metric_key == 'map50':
                score = float(getattr(m, 'map50', None))
            elif metric_key == 'map75':
                score = float(getattr(m, 'map75', None))
            elif metric_key == 'precision':
                score = float(getattr(m, 'mp', None))
            elif metric_key == 'recall':
                score = float(getattr(m, 'mr', None))
        except Exception:
            score = None
    return score if score is not None else -1.0


def evaluate(model, data, imgsz, batch, conf, nms_iou, max_det, project, name, metric_key):
    """
    One validation with current hyperparams; return target metric.
    Patches NMS to force vanilla hard NMS with our (conf, nms_iou, max_det).
    """
    install_hard_nms_patch(conf_thres=conf, iou_thres=nms_iou, max_det_out=max_det)

    results = model.val(
        data=str(data),
        split='val',
        imgsz=imgsz,
        batch=batch,
        conf=conf,        # for consistency with other tuners (also enforced inside patch)
        iou=nms_iou,      # same
        save_txt=False,
        save_conf=False,
        project=str(project),
        name=name,
        verbose=False,
    )
    return fetch_metric(results, metric_key)


# ----------------------------- Main -----------------------------

def main():
    ap = argparse.ArgumentParser("Simulated Annealing tuner for hard NMS: (conf, nms_iou, max_det)")
    ap.add_argument("--weights", type=Path, required=True)
    ap.add_argument("--data", type=Path, required=True)
    ap.add_argument("--imgsz", type=int, default=640)
    ap.add_argument("--batch", type=int, default=4)

    ap.add_argument("--metric", choices=["map5095","map50","map75","precision","recall"],
                    default="map5095", help="Optimize target metric")

    ap.add_argument("--iters", type=int, default=60, help="Total proposals")
    ap.add_argument("--steps-per-temp", type=int, default=5, help="Proposals per temperature level")
    ap.add_argument("--temp0", type=float, default=1.0, help="Initial temperature")
    ap.add_argument("--alpha", type=float, default=0.9, help="Cooling factor")
    ap.add_argument("--seed", type=int, default=0)

    # Ranges (parallel to other tuners for fairness)
    ap.add_argument("--conf-range",    nargs=2, type=float, default=[0.001, 0.3])
    ap.add_argument("--nms-iou-range", nargs=2, type=float, default=[0.3, 0.8])
    ap.add_argument("--max-det-range", nargs=2, type=int,   default=[100, 500])

    # Initial guess (optional)
    ap.add_argument("--init-conf", type=float, default=None)
    ap.add_argument("--init-nms-iou", type=float, default=None)
    ap.add_argument("--init-max-det", type=int, default=None)

    # Step scales (parallel to other tuners)
    ap.add_argument("--step-conf", type=float, default=0.05)
    ap.add_argument("--step-nms-iou", type=float, default=0.05)
    ap.add_argument("--step-max-det", type=float, default=50)

    ap.add_argument("--project", type=Path, default=Path("runs/sa_nms"))
    ap.add_argument("--name", type=str, default="sa_run")
    args = ap.parse_args()

    random.seed(args.seed)
    np.random.seed(args.seed)
    torch.manual_seed(args.seed)

    project = args.project / args.name
    project.mkdir(parents=True, exist_ok=True)
    log_csv  = project / "sa_log.csv"
    best_yaml = project / "best_nms.yaml"

    # Init point
    conf_lo, conf_hi = args.conf_range
    iou_lo,  iou_hi  = args.nms_iou_range
    md_lo,   md_hi   = args.max_det_range

    current = {
        'conf': args.init_conf if args.init_conf is not None else 0.5*(conf_lo + conf_hi),
        'nms_iou': args.init_nms_iou if args.init_nms_iou is not None else 0.5*(iou_lo + iou_hi),
        'max_det': args.init_max_det if args.init_max_det is not None else int(round(0.5*(md_lo + md_hi))),
    }
    current['conf']    = clamp(current['conf'], conf_lo, conf_hi)
    current['nms_iou'] = clamp(current['nms_iou'], iou_lo, iou_hi)
    current['max_det'] = int(round(clamp(current['max_det'], md_lo, md_hi)))

    step_scales = {'conf': args.step_conf, 'nms_iou': args.step_nms_iou, 'max_det': args.step_max_det}
    ranges = {'conf': (conf_lo, conf_hi), 'nms_iou': (iou_lo, iou_hi), 'max_det': (md_lo, md_hi)}

    # Load model once
    model = YOLO(str(args.weights))

    # Initial evaluation
    cur_score = evaluate(
        model, args.data, args.imgsz, args.batch,
        current['conf'], current['nms_iou'], current['max_det'],
        project, "sa_eval_init", args.metric
    )
    best = dict(current)
    best_score = cur_score

    # CSV header (same columns style as other tuners)
    with open(log_csv, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["iter","T","conf","nms_iou","max_det","score","accept","is_best","run_name"])

    T = args.temp0
    it = 0
    temp_level = 0
    while it < args.iters:
        temp_level += 1
        for _ in range(args.steps_per_temp):
            if it >= args.iters:
                break
            it += 1
            cand = propose(current, ranges, step_scales)
            run_name = f"sa_eval_t{temp_level}_i{it}"
            new_score = evaluate(
                model, args.data, args.imgsz, args.batch,
                cand['conf'], cand['nms_iou'], cand['max_det'],
                project, run_name, args.metric
            )
            ap_ = accept_prob(cur_score, new_score, T)
            accepted = (random.random() < ap_)
            if accepted:
                current = cand
                cur_score = new_score
            is_best = False
            if new_score > best_score:
                best, best_score = cand, new_score
                is_best = True
            with open(log_csv, "a", newline="", encoding="utf-8") as f:
                w = csv.writer(f)
                w.writerow([it, f"{T:.6f}",
                            f"{cand['conf']:.6f}", f"{cand['nms_iou']:.6f}", cand['max_det'],
                            f"{new_score:.6f}", int(accepted), int(is_best), run_name])
        T *= args.alpha

    # Save best hyperparams (same schema as other tuners)
    out = {
        "metric": args.metric,
        "best_score": float(best_score),
        "best_params": {
            "conf": float(best['conf']),
            "nms_iou": float(best['nms_iou']),
            "max_det": int(best['max_det']),
        },
        "ranges": {
            "conf": list(args.conf_range),
            "nms_iou": list(args.nms_iou_range),
            "max_det": list(args.max_det_range),
        },
        "sa": {
            "iters": args.iters,
            "steps_per_temp": args.steps_per_temp,
            "temp0": args.temp0,
            "alpha": args.alpha,
            "seed": args.seed
        },
        "weights": str(args.weights),
        "data": str(args.data),
        "imgsz": args.imgsz,
        "batch": args.batch,
        "project": str(project),
    }
    with open(best_yaml, "w", encoding="utf-8") as f:
        yaml.safe_dump(out, f, allow_unicode=True, sort_keys=False)

    print("\n===== SA (Hard NMS) Finished =====")
    print(f"Best {args.metric}: {best_score:.6f}")
    print(f"Best params: conf={best['conf']:.6f}, nms_iou={best['nms_iou']:.6f}, max_det={best['max_det']}")
    print(f"Log CSV: {log_csv}")
    print(f"Best YAML: {best_yaml}")


if __name__ == "__main__":
    main()
